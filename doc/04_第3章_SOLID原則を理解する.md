...
本章では、さらに一歩進んで、より具体的で実践的なオブジェクト指向設計の原則であるSOLID原則について掘り下げていきます。SOLID原則は、ロバート・C・マーティン（通称Uncle Bob）によって提唱された、以下の5つの原則の頭文字をとったものです。
...
単一責任の原則（SRP）は、「クラス（やモジュール、関数）を変更する理由は、一つだけであるべきだ」と述べています。
...
本体: コピー機本体
  構築時に 原稿読み取り部 と 印字部 を受け取り保持する
  実行(原稿ID, 用紙設定)
    → 読み取り部へ 読み取る(原稿ID) を依頼し画像データを得る
    → 印字部へ 印字する(取得した画像, 用紙設定) を依頼する
```
...
本体: 万能コピー機本体
  実行(原稿ID, 用紙設定)
    → スキャナに直接アクセスして画像を取得
    → 同じ本体で画像補正まで実行
    → プリンタに出力指示を送り印字を完了させる
    → ついでに見積もりログまで記録する
```

読み取りも印字もログ記録も一つのクラスに押し込めてしまうと、どの変更でも本体を編集せざるを得なくなります。SRP違反は、AIが生成したコードの差分を見極める際にも障害になります。

## O: Open-Closed Principle（開放/閉鎖の原則）

### 原則の説明

開放/閉鎖の原則（OCP）は、「ソフトウェアのエンティティ（クラス、モジュールなど）は、拡張に対しては開いており、修正に対しては閉じているべきだ」と述べています。
...
一方、このA3対応のために、コピー機本体の既存のソースコードを直接修正する必要があったとしたら、どうでしょうか。その修正が、既存のA4印刷機能にバグを埋め込んでしまうかもしれません。OCPは、このようなリスクを避けるため、既存のコードは「修正に対して閉じている」べきだと主張します。
...
制御部: コピー制御
  構築時に 利用する給紙ユニット を注入して保持する
  実行()
    → 給紙ユニットへ 次の用紙() を依頼
    → 取得した用紙設定を印字部に渡して印刷を行う
```
...
本体: 条件分岐コピー制御
  実行(モード種別)
    → モードがA4なら A4給紙ユニットを直接呼び出す
    → それ以外なら A3給紙ユニットを直接呼び出す
    → 新しい用紙サイズが増えるたびに分岐が追加される
```

本体の条件分岐を増やしてしまうと、新しいサイズを追加するたびに本体コードを変更する必要が出てきます。OCPを守ることで、AIにも「新しい給紙ユニットを追加するだけで良い」と伝えられるのです。

## L: Liskov Substitution Principle（リスコフの置換原則）

### 原則の説明

リスコフの置換原則（LSP）は、「派生型は、その基底型と置換可能でなければならない」と述べています。

これは、OCPで触れた「抽象」と「継承」を、より正しく使うための原則です。あるクラス（基底型）を継承したクラス（派生型）は、基底型の代わりに使っても、プログラムの振る舞いを変えてはならない、という意味です。
...
LSPを守るということは、継承関係にあるクラス群が、同じ「契約」を守ることを意味します。これにより、クライアント（コピー機本体）は、安心して抽象（給紙ユニットインターフェース）に依存することができるのです。
...
実装: CCD読み取りヘッド ← 読み取りヘッド
  読み取る() は CCDセンサーで画像を取得する
  キャリブレーション() は CCDセンサーの校正を呼び出す

実装: CIS読み取りヘッド ← 読み取りヘッド
  読み取る() は CISセンサーで画像を走査する
  キャリブレーション() は CISセンサーの調整機能を呼び出す
```
...
```text
実装: 手差し読み取りヘッド ← 読み取りヘッド
  読み取る() がエラーを投げてしまい、契約どおり画像データを返さない
  キャリブレーション() も空実装のまま放置されている
```

この派生型は、基底の契約で保証された「読み取る」操作を破り、呼び出し側に想定外の例外を投げています。LSPが満たされていないと、AIが提案した差し替え案を採用した途端に本番障害を招くリスクがあります。

## I: Interface Segregation Principle（インターフェース分離の原則）

### 原則の説明

インターフェース分離の原則（ISP）は、「クライアントに、自身が使わないメソッドへの依存を強制してはならない」と述べています。
...
```text
悪い例: FinishingUnit 契約
  操作 綴じる()
  操作 折る()
  操作 穴を開ける()
→ すべての仕上げ機能を一つに詰め込んでしまい、利用側に不要な操作まで押しつけている
```
...
ISPは、このような「太ったインターフェース」を分割することを推奨します。
...
```text
実装: ステープルユニット ← Stapler
  綴じる() を呼び出すとホチキス機構が綴じ処理を行う

制御部: 仕上げ制御
  構築時に 必要な仕上げ機能ごとの契約 (例: Stapler, Puncher) を任意で受け取る
  綴じ処理()
    → Stapler が渡されていれば 綴じる() を呼び出す
    → 不要な機能には依存しない
```

依存先を機能ごとに選べるため、AIに「パンチ機能だけを追加して」と頼むと `Puncher` 実装と配線の追加だけで済むようになります。

SRPがクラスの責任を分離する原則だったのに対し、ISPはインターフェースの責任を分離する原則と考えることができます。

## D: Dependency Inversion Principle（依存性逆転の原則）

### 原則の説明

依存性逆転の原則（DIP）は、SOLID原則の集大成とも言える、非常に重要な原則です。DIPは、2つのことを主張しています。
...
この原則を実現する具体的なテクニックが、依存性注入（Dependency Injection, DI） です。コピー機本体は、自身で「レーザー印字部」のインスタンスを生成するのではなく、外部から「印字部インターフェースを実装した、何らかのインスタンス」を注入される形で受け取ります。これにより、上位モジュールと下位モジュールの疎結合が実現されるのです。
...
実装: レーザー印字部 ← 印字部
  印字する() は レーザー方式の印字機構に処理を委ねる

実装: インクジェット印字部 ← 印字部
  印字する() は インク噴射方式の印字機構に処理を委ねる

本体: コピー機本体
  構築時に 印字部(抽象) を受け取って保持する
  実行(画像データ, 用紙設定)
    → 保持している印字部へ 印字する() を依頼するだけで完結
```
...
```text
本体: 密結合コピー機本体
  起動時に自分で レーザー印字部 の具象クラスを生成して保持する
  実行(画像データ, 用紙設定)
    → 直接その具象クラスへ印字処理を依頼する
    → 方式を変えるたびに本体側のコードも書き換えが必要になる
```

具象クラスを直接生成・操作してしまうと、印字方式を変えるたびに本体の修正が必要になります。DIP違反は、AIに「インクジェット対応して」と依頼した際に、本体の大改修を誘発します。

## まとめ

本章では、SOLID原則という、堅牢で柔軟なソフトウェアを設計するための5つの柱について、コピー機を例に解き明かしました。

-   SRP: クラスの責任を一つに絞り、変更に強くする。
-   OCP: 抽象を利用し、修正ではなく拡張で機能追加に対応する。
-   LSP: 継承の「契約」を守り、置換可能性を保証する。
-   ISP: インターフェースを適切に分割し、無駄な依存をなくす。
-   DIP: 抽象に依存することで、モジュール間の結合を疎にする。
...
- スタートアップ初期: まずはSRPとDIPで差し替えやすさを確保し、他の原則はプロダクトマーケットフィット後に拡張する。
- エンタープライズ大規模開発: 全原則を明文化し、レビューでチェックリスト化しておく。
- プロトタイプやPoC: KISSとSRPの遵守を最低ラインとし、OCP/LSPは追加フェーズで整える。
